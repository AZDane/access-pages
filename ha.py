...
class BrokerHomeAssistantClient(HomeAssistantClient):
    ...
    def revoke_guest_sessions(self, page_id, grant_id):
        if self.broker_role != "admin":
            raise HomeAssistantError("Guest session cleanup requires admin authority")
        return self._request(
            "POST", "/v1/revoke-guest-sessions",
