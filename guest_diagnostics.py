...
def timing_from_headers(headers, operation, *, required=False, internal_rpc=False):
    ...
    if internal_rpc:
        ...
        if len(operations) != 1 or operations[0] not in OPERATIONS:
            raise ValueError("Invalid request timing")
        if operation == "action" and operations[0] != "action":
            raise ValueError("Invalid request timing")
    ...
    if not 0 < started <= now or (rpc and not started <= rpc <= now):
        raise ValueError("Invalid request timing")
    return RequestTiming(request_id, started, operation, rpc)


def action_remaining(default):
    timing = CURRENT.get()
    return min(default, timing.remaining()) if timing and timing.operation == "action" else default
...
def guest_operation(method, path, bootstrap=False):
    ...
    if len(parts) >= 5 and parts[1] == "g":
        ...
        if len(parts) == 7 and parts[4:6] == ["api", "access"]:
            return "state"
        if len(parts) == 9 and parts[4:6] == ["api", "access"]:
            if parts[7] in {"verification", "camera"}:
                return parts[7]
            if method == "POST":
                return "action"
    return "other"
