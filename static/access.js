import {AccessConnectionError, createAccessApi} from "./access-api.js";
...
function responseError(
  ...
  if (response.status >= 500) {
    return new AccessConnectionError(message);
  }
...
async function poll() {
  // Visibility/online events may coincide with a scheduled refresh.
  clearTimeout(pollTimer);
  pollTimer = null;
  if (statusRequest || commandInProgress || verificationPending || document.hidden ||
      !navigator.onLine || accessEnded) return;
  try {
    await fetchPage();
  } catch (error) {
    if (error instanceof AccessEndedError) {
