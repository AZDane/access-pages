"""Saved-page action validation and dispatch for administrative preview."""
...
def execute_public_action(
    ...
    try:
        deadline = getattr(handler, "action_deadline", None)
        if deadline:
        ...
        ha_client.call_service(
            ...
            action_id=action_id,
            **({"grant_deadline": deadline} if deadline else {}),
        )
    except HomeAssistantError as error:
        handler._send_ha_error(error)
        return
