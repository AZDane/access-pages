...
from upstream_cleanup import CleanupQueue
...
class PageStore:
    def __init__(self, directory: Path, file_mode: int = 0o600):
        ...
        self._lock = RLock()
        self.cleanup = CleanupQueue(self.directory / ".upstream-cleanup.sqlite3")
    ...
    def _load_unlocked(self, page_id: str) -> dict:
        ...
        if self._revoking_path(page_id).exists():
            page["access_grants"] = []
        return page

    def _revoking_path(self, page_id: str) -> Path:
        validate_page_id(page_id)
        return self.directory / f"{page_id}.revoking"

    def _recover_revocation_unlocked(self, page_id: str) -> None:
        marker = self._revoking_path(page_id)
        if not marker.exists():
            return
        try:
            grants = validate_access_grants(
                json.loads(marker.read_text(encoding="utf-8"))["grants"]
            )
        except (OSError, ValueError, KeyError, TypeError) as error:
            raise PageConfigError("Invalid durable revocation record") from error
        for grant in grants:
            self.cleanup.enqueue(page_id, grant)
        path = self._path(page_id)
        if path.exists():
            payload = validate_page(
    ...
    def recover_pending_revocations(self) -> None:
        with self._lock:
            self.ensure_directory()
            for marker in sorted(self.directory.glob("*.revoking")):
                self._recover_revocation_unlocked(marker.stem)

    def revoke_page_access_grants(self, page_id: str) -> tuple[dict, list[dict]]:
        """Durably mask local access before queueing each remote cleanup."""
        with self._lock:
            self._recover_revocation_unlocked(page_id)
            page = self._load_unlocked(page_id)
            grants = list(page["access_grants"])
            if grants:
                self._write(self._revoking_path(page_id), {"grants": grants})
                self._recover_revocation_unlocked(page_id)
                page["access_grants"] = []
            return page, grants
    ...
    def revoke_all_access_grants(self) -> tuple[int, list[dict]]:
        ...
        with self._lock:
            self.recover_pending_revocations()
            self.ensure_directory()
            updated_pages = 0
            revoked_grants = []
            for path in sorted(self.directory.glob("*.json")):
                ...
                if not grants:
                    continue
                self.revoke_page_access_grants(page["id"])
...
def validate_access_grants(raw_grants: object) -> list[dict]:
    ...
    for raw in raw_grants:
        ...
        flow = raw.get("credential_flow")
        if flow != "bootstrap-v1":
            raise PageConfigError("Invalid guest credential flow")
        if not re.fullmatch(r"grant_[A-Za-z0-9_-]{16}", grant_id):
            raise PageConfigError("Invalid scoped grant ID")
        upstream_scope = raw.get("upstream_scope", "page")
        if upstream_scope not in {"page", "guest"}:
            raise PageConfigError("Invalid upstream resource scope")
        if upstream_scope == "guest" and not raw.get("resource_crid"):
            raise PageConfigError("Guest resource requires scoped credentials and a CRID")
        ...
        grants.append(
            {
                ...
                "qurl_link": str(raw.get("qurl_link", "")).strip(),
                "qurl_site": str(raw.get("qurl_site", "")).strip(),
                "resource_id": str(raw.get("resource_id", "")).strip(),
                "qurl_id": str(raw.get("qurl_id", "")).strip(),
                ...
                "credential_flow": flow,
                "resource_crid": str(raw.get("resource_crid", "")).strip(),
