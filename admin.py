...
def handle_get(handler, parsed, path, runtime):
    if path == "/api/admin/notification-options":
        ...
        except (runtime.EmailConfigError, OSError) as error:
    ...
    if path == "/api/admin/pages":
        ...
        handler._send_json(
            ...
            {
                "pages": runtime.PAGE_STORE.list_pages(),
                "layerv_api_configured": runtime.LAYERV_CLIENT.configured,
                "qurl_max_lifetime_days": runtime.QURL_MAX_LIFETIME_DAYS,
            },
        )
        return
...
def handle_post(handler, path, payload, runtime):
    ...
    if path.startswith("/api/admin/pages/") and path.endswith("/qurls"):
        if not handler._require_admin():
            return

        page_id = path.removeprefix("/api/admin/pages/")
        page_id = page_id.removesuffix("/qurls").strip("/")
        if not page_id or "/" in page_id:
            handler._send_json(404, {"error": "not found"})
            return

        with runtime.page_action_lock(page_id):
            handler._create_qurl(page_id, payload)
        return
    handler._send_json(404, {"error": "not found"})
...
def handle_delete(handler, path, runtime):
    ...
    if path.startswith("/api/admin/pages/") and "/qurls/" in path:
        remainder = path.removeprefix("/api/admin/pages/").strip("/")
        parts = remainder.split("/")
        if len(parts) != 3 or parts[1] != "qurls":
            handler._send_json(404, {"error": "not found"})
            return
    ...
    if path.startswith("/api/admin/pages/"):
        ...
        try:
            with runtime.page_action_lock(page_id):
                ...
                for grant in grants:
                    try:
                        ...
                        runtime.audit("verification_recipient_cleanup_failed", page_id=page_id, grant_id=grant["id"])
                    try:
            ...
            remote_failures = []
            for grant in grants:
                runtime.cleanup_guest_sessions(page_id, grant["id"])
                qurl_id = grant.get("qurl_id", "")
                if not qurl_id:
                    remote_failures.append({"grant_id": grant["id"], "error": "Missing qURL ID"})
                    continue
                try:
                    runtime.LAYERV_CLIENT.delete_qurl(
                        resource_id=grant.get("resource_id", ""),
                        qurl_id=qurl_id,
                        page_id=page_id,
                        grant_id=grant["id"],
                    )
                except runtime.LayerVError as error:
